        ...
        return (
            <div className="bg-white rounded-lg shadow p-8">
                <div className="text-center text-gray-500">
                    ...
                    <p className="text-sm mt-1">Không tìm thấy người dùng phù hợp với bộ lọc</p>
                </div>
            </div>
        );
    }
            ...
            <table className="w-full">
                <thead className="bg-gray-50 border-b">
                    <tr>
                        ...
                        <th className="px-6 py-3 text-left text-sm font-semibold text-gray-700">Thông tin bổ sung</th>
                ...
                <tbody className="divide-y divide-gray-200">
                    {users.map((user) => (
                        <tr key={user._id} className="hover:bg-gray-50 transition">
                            {/* UID Column - sử dụng userId từ API */}
                            ...
                            {/* Phone Number Column - sử dụng phoneNumber từ API */}
                            ...
                            <td className="px-6 py-4">
                                <Badge 
                                    variant={
                                        ...
                                        user.role === 'driver' ? 'default' :
                                        user.role === 'parent' ? 'secondary' : 
                                        user.role === 'manager' ? 'outline' : 'outline'
                                    }
                            ...
                            {/* Additional Info Column - hiển thị thông tin theo role */}
                            <td className="px-6 py-4 text-sm text-gray-700">
                                    ...
                                    <div className="space-y-1">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-xs">
                                                <span className="font-medium">GPLX:</span> {user.driverInfo.licenseNumber}
                                            </span>
                                        </div>
                                        {user.driverInfo.vehiclePlate && (
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-xs">
                                                    <span className="font-medium">Biển số:</span> {user.driverInfo.vehiclePlate}
                                                </span>
                                            </div>
                                        )}
                                    </div>
